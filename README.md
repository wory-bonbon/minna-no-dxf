...
- `PDF`/`SVG` は `表示(全体/選択)` と `ALL` のスコープを切替えられます。
- `PNG` は現状「表示(全体/選択)」の範囲のみ書き出し可能です（`ALL` 指定は無効）。
- 目盛り描画（スケールバー）の ON/OFF も可能です。
- `PNG` は透過背景・300dpi 指定、`SVG` は1:1 実寸出力で stroke=0.17mm。サイドパネルにも 1:1 固定の SVG ショートカットを用意
...
| **書き出し設定** | 範囲選択、目盛り、PDF/SVG/PNG | `PDF`/`SVG`は`表示(全体/選択)`と`ALL`を切替可能、`PNG`は`表示(全体/選択)`のみ対応。目盛りON/OFF |
...
- PNG 書き出しは表示範囲（全体／選択）のみ対応。`ALL` 指定での全体書き出しには対応していません
